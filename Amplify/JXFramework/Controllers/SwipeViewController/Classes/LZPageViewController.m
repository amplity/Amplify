...
- (NSMutableArray *)controllers{
    if (!_controllers) {
        ...
        for (int i = 0; i < _controllersClass.count; i ++) {
            ...
            vc.inputViewData =_inputViewDatas[i];
...
- (instancetype)initWithTitles:(NSArray *)titlesArray controllersClass:(NSArray *)controllersClass inputViewDatas:(NSArray*)inputViewDatas
{
    if (self = [super init]) {
        ...
        self.inputViewDatas = inputViewDatas;
...
- (void)addCollectionPage{
    ...
    line.frame = CGRectMake(0, self.pageBarHeight - 1.5, self.lineWidth, 1.5);
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.line.frame = CGRectMake((0 + indexPath.row) * self.lineWidth, self.pageBarHeight - 1.5, self.lineWidth, 1.5);
    }];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.line.frame = CGRectMake(index *self.lineWidth, self.pageBarHeight - 1.5, self.lineWidth, 1.5);
    }];
...
- (void)pageContentView:(LZPageContentView *)pageContentView didClickBtnIndex:(NSInteger)index
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.line.frame = CGRectMake((0 + index) *self.lineWidth, self.pageBarHeight - 1.5, self.lineWidth, 1.5);
    }];
}
